...
export default function Home() {
  ...
  // Moved fetchQuote function outside useEffect
  const fetchQuote = async () => {
    const timestamp = new Date().getTime(); // Move timestamp here so it refreshes with each fetch
    try {
      setIsLoading(true);
      ...
      <nav className="absolute top-6 right-6">
        ...
        {isLoading ? (
          <button
            onClick={fetchQuote}
            disabled
            className="backdrop-blur-sm bg-white/30 font-bold py-2 px-4 rounded disabled:bg-white/15"
          >
            Loading...
          </button>
        ) : (
          <button
            onClick={fetchQuote}
            className="backdrop-blur-sm bg-white/30 font-bold py-2 px-4 rounded"
          >
            Get New Quote
          </button>
        )}
